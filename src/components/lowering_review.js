...
class LoweringReview extends Component {
  ...
  async handleEventUpdate(formProps) {
    ...
    this.props.updateLoweringReplayEvent(formProps.id)
  }
        ...
        if (index >= (this.state.activePage - 1) * maxEventsPerPage && index < this.state.activePage * maxEventsPerPage) {
          ...
          let eventDetails = (
            <OverlayTrigger placement='left' overlay={<Tooltip id={`commentTooltip_${event.id}`}>View Details</Tooltip>}>
              <FontAwesomeIcon onClick={() => this.handleEventShowDetailsModal(event)} icon='window-maximize' fixedWidth />
            </OverlayTrigger>
          )
...
LoweringReview.propTypes = {
  ...
  updateLoweringReplayEvent: PropTypes.func.isRequired,

...
class Users extends Component {
  ...
  renderAddUserButton() {
    if (!this.props.showform) {
      return (
        <Button variant='primary' size='sm' onClick={() => this.handleUserCreate()} disabled={!this.props.user_id}>
          Add User
        </Button>
      )
    }
  ...
  renderUsers() {
    ...
    return users.map((user) => {
      ...
      const className = this.props.user_id === user.id ? 'text-warning' : ''
  ...
  renderSystemUsers() {
    ...
    return system_users.map((user) => {
      ...
      if (user.system_user) {
        return (
          <tr key={user.id}>
            <td style={style} className={this.props.user_id === user.id ? 'text-warning' : ''}>
              {user.username}
            </td>
